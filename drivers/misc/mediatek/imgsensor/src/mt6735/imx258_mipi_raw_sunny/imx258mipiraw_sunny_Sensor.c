...
extern int is_sunny_camera(void);
#endif /*IMX258_SUNNY_CHECK_VENDOR_ID*/
#ifdef IMX258_SUNNY_USE_WB_OTP
extern int imx258_sunny_update_otp_wb(void);
#endif
#ifdef IMX258_SUNNY_USE_LSC_OTP
extern int imx258_sunny_update_otp_lenc(void);
#endif /*IMX258_SUNNY_USE_LSC_OTP*/
#ifdef IMX258_SUNNY_OTP_DEBUG
extern void IMX258_Test_OTP(void);
#endif
...
static kal_uint16 imx258_sunny_type = 0;/*0x00=HDR type, 0x10=binning type*/
...
kal_uint16 IMX258MIPI_SUNNY_sensorGainMapping[IMX258MIPI_MaxGainIndex][2] ={
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
extern bool read_imx258_sunny_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258_sunny_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size);
...
static void load_imx258_SPC_Data(void)
{
	...
	{
		if(!read_imx258_sunny_eeprom_SPC(0x023B,IMX258MIPI_SPC_Data,126))
		{
...
static kal_uint32 return_sensor_id(void)
{
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 ...
			 {
			 	 imx258_sunny_type = read_cmos_sensor(0x0A2E);
				 LOG_INF("imx258 type = 0x%x(0x00=HDR,0x10=binning)", imx258_sunny_type);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX258MIPI_MaxGainIndex; i++) {
        if(gain <= IMX258MIPI_SUNNY_sensorGainMapping[i][0]){
            break;
        }
    }
    if(gain != IMX258MIPI_SUNNY_sensorGainMapping[i][0])
            LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX258MIPI_SUNNY_sensorGainMapping[i][0]);
    return IMX258MIPI_SUNNY_sensorGainMapping[i][1];
}
...
{
	if(imx258_sunny_type == IMX258_HDR_TYPE)
	{
	...
	else if(imx258_sunny_type == IMX258_BINNING_TYPE)
	{
		write_cmos_sensor(0x94C7,0xFF);
		write_cmos_sensor(0x94C8,0xFF);
		write_cmos_sensor(0x94C9,0xFF);
...
#ifdef IMX258_SUNNY_USE_WB_OTP
	imx258_sunny_update_otp_wb();
#endif
#ifdef IMX258_SUNNY_USE_LSC_OTP
	imx258_sunny_update_otp_lenc();
#endif /*IMX258_SUNNY_USE_LSC_OTP*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            *sensor_id = return_sensor_id();
			...
			#ifdef IMX258_SUNNY_CHECK_VENDOR_ID
				if(0==is_sunny_camera())
				{
					pr_err("[yixuhong] it's not sunny camera! \n");
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				*sensor_id = imgsensor_info.sensor_id+1;  //sunny camera return 0x259
			#endif /*IMX258_SUNNY_CHECK_VENDOR_ID*/
				return ERROR_NONE;
			}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258_sunny_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
