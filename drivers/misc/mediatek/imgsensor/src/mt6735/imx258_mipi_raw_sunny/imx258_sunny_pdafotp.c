...
#include "imx258mipiraw_sunny_Sensor.h"

#ifdef IMX258_SUNNY_OTP
...
static BYTE imx258_sunny_eeprom_data[DATA_SIZE]= {0};
static bool get_done = false;
static int last_size = 0;
static int last_offset = 0;

static kal_uint16 imx258_sunny_read_eeprom(kal_uint16 addr, kal_uint8 write_id)
{
...
void IMX258_Test_OTP(void)
{
    ...
    {
        value = imx258_sunny_read_eeprom(addr,write_id);
        LOG_INF("%s:write_id = %x, addr:%x, value:%x \n",__func__,write_id,addr,value);
    }
}
#endif /*IMX258_SUNNY_OTP_DEBUG*/

#ifdef IMX258_SUNNY_CHECK_VENDOR_ID
int is_sunny_camera(void)
{
	kal_uint8 vendor_id = imx258_sunny_read_eeprom(0x01,IMX258_EEPROM_READ_ID);
	if(0x1 == vendor_id)//sunny = 0x1
	{
		return 1;
	}
	return 0;
}
#endif /*IMX258_SUNNY_CHECK_VENDOR_ID*/

#if defined(IMX258_SUNNY_USE_WB_OTP)
// R/G and B/G of typical camera module is defined here
static int RG_Ratio_Typical_Sunny = 0x01f0;//0x01f0
static int BG_Ratio_Typical_Sunny = 0x0267;//
static int GG_Ratio_Typical_Sunny = 0x0400;
...
static int check_otp_wb(void)
{
    ...
    flag_Pro= imx258_sunny_read_eeprom(0x01,write_id);
    flag_AWB= imx258_sunny_read_eeprom(0x1C,write_id);

    LOG_INF("check_otp_wb, flag_Pro = %x, flag_AWB = %x\n", flag_Pro, flag_AWB);
    if((0x01 == flag_Pro)&&(flag_AWB&0x01))
        return 1;
    else
        return 0;
}
...
static int read_otp_wb( struct otp_struct * otp_ptr)
{
    ...
    for(i=0; i<6; i++)
    {
        otp_data[i] = imx258_sunny_read_eeprom(0x1D+i,0xA0);
		#ifdef IMX258_SUNNY_OTP_DEBUG
...
//      1, no OTP
int imx258_sunny_update_otp_wb(void)
{
    ...
    LOG_INF("imx258_sunny_update_otp_wb,\n");
    otp_valid = check_otp_wb();
    if(otp_valid==0)
    {
        // no valid wb OTP data
        LOG_INF("imx258_sunny_update_otp_wb,no valid wb OTP data\n");
        return 1;
    }
	...
	{
		LOG_INF("imx258_sunny_update_otp_wb get bad otp data !\n current_otp.rg_ratio=0x%x,current_otp.bg_ratio=0x%x",current_otp.rg_ratio,current_otp.bg_ratio);
		return 1;
	}
	if (current_otp.bg_ratio < BG_Ratio_Typical_Sunny) 
	{
        if (current_otp.rg_ratio< RG_Ratio_Typical_Sunny) 
		{
            G_gain = GAIN_DEFAULT;
            B_gain = (GAIN_DEFAULT * BG_Ratio_Typical_Sunny) / current_otp.bg_ratio;
            R_gain = (GAIN_DEFAULT * RG_Ratio_Typical_Sunny) / current_otp.rg_ratio;
        }
		else 
		{
            R_gain = GAIN_DEFAULT;
            G_gain = (GAIN_DEFAULT * current_otp.rg_ratio) / RG_Ratio_Typical_Sunny;
            B_gain = (G_gain * BG_Ratio_Typical_Sunny) /current_otp.bg_ratio;
    	}
    }else 
    {
        if (current_otp.rg_ratio < RG_Ratio_Typical_Sunny) 
		{
            B_gain = GAIN_DEFAULT;
            G_gain = (GAIN_DEFAULT * current_otp.bg_ratio)/ BG_Ratio_Typical_Sunny;
            R_gain = (G_gain * RG_Ratio_Typical_Sunny)/ current_otp.rg_ratio;
        }
		else 
		{
            G_gain_B = (GAIN_DEFAULT * current_otp.bg_ratio) / BG_Ratio_Typical_Sunny;
            G_gain_R = (GAIN_DEFAULT * current_otp.rg_ratio) / RG_Ratio_Typical_Sunny;
...
#if 0 //yixuhong 20151221
	if (current_otp.gg_ratio <= GG_Ratio_Typical_Sunny)
	{
		G_gain_R = G_gain;
		G_gain_B = G_gain * GG_Ratio_Typical_Sunny / current_otp.gg_ratio;		 
    }
	else 
	{
		G_gain_B = G_gain;
		G_gain_R = G_gain * current_otp.gg_ratio /GG_Ratio_Typical_Sunny;
		R_gain = R_gain * current_otp.gg_ratio /GG_Ratio_Typical_Sunny;
		B_gain = B_gain * current_otp.gg_ratio /GG_Ratio_Typical_Sunny;
	}
#else
	G_gain_R = G_gain; 
	G_gain_B = G_gain;
...
#endif /*IMX258_SUNNY_USE_WB_OTP*/

#if defined(IMX258_SUNNY_USE_LSC_OTP)
// For HW
...
static int read_otp_lenc(int index, u32 * otp_ptr)
{
    ...
    for(i = 0; i <= 504; i++)
    {
        otp_ptr[i] = imx258_sunny_read_eeprom(0x41+i,IMX258_EEPROM_READ_ID);
		#ifdef IMX258_SUNNY_OTP_DEBUG
...
int imx258_sunny_update_otp_lenc(void)
{
    int otp_valid;
	u32* otp_data_ptr;
    ...
    {
        LOG_INF("imx258_sunny_update_otp_lenc malloc memory fail\n");
        return 1;
    }
...
#endif /*IMX258_SUNNY_USE_LSC_OTP*/
...
bool read_imx258_sunny_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size){
	addr = 0x0763;
	size = 1404;
	...
	if(!get_done || last_size != size || last_offset != addr) {
		if(!_read_imx258_eeprom(addr, imx258_sunny_eeprom_data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			return false;
	...
	memcpy(data, imx258_sunny_eeprom_data, size);
    return true;
}

bool read_imx258_sunny_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size){
	int i;
	addr = 0x023B;//0x0F73;
	size = 126;
