...
extern void IMX179MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
...
uint32_t IMX179_RG_Ratio_typical_Sunny=0x022E;//25F;//607
uint32_t IMX179_BG_Ratio_typical_Sunny=0x024E;//230;//560
uint32_t IMX179_GG_Ratio_typical_Sunny=0x0400;//1023
...
int imx179_update_awb_gain()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
    ...
    #ifdef IMX179_OTP_DEBUG
    printk("[Sunny] Bruceimx179_R_gain1 == 0x%x\n",imx179_R_gain1);
    printk("[Sunny] imx179_G_gain1 == 0x%x\n",imx179_G_gain1);
    printk("[Sunny] imx179_B_gain1 == 0x%x\n",imx179_B_gain1);
    #endif
