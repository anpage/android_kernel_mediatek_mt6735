...
#define IMX258MIPI_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1,  imgsensor.i2c_write_id)
...
void IMX258_Test_OTP(void)
{
    ...
    for(addr = 0; addr <= 0x1b; addr++)
    {
        ...
        pr_err("%s:write_id = %x, addr:%x, value:%x \n",__func__,write_id,addr,value);
    }
}
...
int RG_Ratio_Typical = 0x0230;
int BG_Ratio_Typical = 0x0248;
int GG_Ratio_Typical = 0x0405 ;
...
static int check_otp_wb(void)
{
    ...
    flag_Pro= imx258_liteon_read_eeprom(0x03,write_id);
    flag_AWB= imx258_liteon_read_eeprom(0x04,write_id);
...
static int read_otp_wb( struct otp_struct * otp_ptr)
{
    ...
    {
        otp_data[addr] = imx258_liteon_read_eeprom(addr,0xA0);
		#ifdef IMX258_LITEON_OTP_DEBUG
			LOG_INF("read_otp_wb addr=0x%x value=0x%x \n",addr,otp_data[addr]);
		#endif
    }

    otp_ptr->rg_ratio = (otp_data[6]<<8)|otp_data[7];
    otp_ptr->bg_ratio = (otp_data[8]<<8)|otp_data[9];
    otp_ptr->gg_ratio = (otp_data[10]<<8)|otp_data[11];
...
int imx258_liteon_update_otp_wb(void)
{
	...
	if (current_otp.bg_ratio < BG_Ratio_Typical) {
        if (current_otp.rg_ratio< RG_Ratio_Typical) {
            G_gain = GAIN_DEFAULT;
            B_gain = (GAIN_DEFAULT * BG_Ratio_Typical) / current_otp.bg_ratio;
            R_gain = (GAIN_DEFAULT * RG_Ratio_Typical) / current_otp.rg_ratio;
        }else {
            R_gain = GAIN_DEFAULT;
            G_gain = (GAIN_DEFAULT * current_otp.rg_ratio) / RG_Ratio_Typical;
            B_gain = (G_gain * BG_Ratio_Typical) /current_otp.bg_ratio;
    }
    }else {
        if (current_otp.rg_ratio < RG_Ratio_Typical) {
            B_gain = GAIN_DEFAULT;
            G_gain = (GAIN_DEFAULT * current_otp.bg_ratio)/ BG_Ratio_Typical;
            R_gain = (G_gain * RG_Ratio_Typical)/ current_otp.rg_ratio;
        }else {
            G_gain_B = (GAIN_DEFAULT * current_otp.bg_ratio) / BG_Ratio_Typical;
            G_gain_R = (GAIN_DEFAULT * current_otp.rg_ratio) / RG_Ratio_Typical;
	...
	if (current_otp.gg_ratio <= GG_Ratio_Typical)
		{G_gain_R = G_gain;
	         G_gain_B = G_gain * GG_Ratio_Typical / current_otp.gg_ratio;		 
              }
	else 
		{G_gain_B = G_gain;
	         G_gain_R = G_gain * current_otp.gg_ratio /GG_Ratio_Typical;
		  R_gain = R_gain * current_otp.gg_ratio /GG_Ratio_Typical;
		  B_gain = B_gain * current_otp.gg_ratio /GG_Ratio_Typical;
		}

    LOG_INF("[liujing] R_gain=0x%x,G_gain_R=0x%x, G_gain_B=0x%x,B_gain=0x%x",R_gain,G_gain_R,G_gain_B,B_gain );
...
{
    kal_uint16 addr = 0;
    kal_uint8 write_id = 0xA2;

    for(addr = 0; addr <= 0xFF; addr++)
    {
        otp_ptr[addr] = imx258_liteon_read_eeprom(addr,write_id);
		#ifdef IMX258_LITEON_OTP_DEBUG
        LOG_INF("%s:write_id = %x, addr:%x, value:%x \n",__func__,write_id,addr,otp_ptr[addr]);
        #endif
    }
    ...
    for(addr = 0; addr <= 0xFF; addr++)
    {
        otp_ptr[addr+0xFF] = imx258_liteon_read_eeprom(addr,write_id);
        #ifdef IMX258_LITEON_OTP_DEBUG
        LOG_INF("%s:write_id = %x, addr:%x, value:%x \n",__func__,write_id,addr,otp_ptr[addr+0xFF]);
        #endif
    }
    return 0;
}
...
static int update_lenc(u32 * otp_ptr)
{
    ...
    IMX258MIPI_write_cmos_sensor(0x0700, 0x00);
    IMX258MIPI_write_cmos_sensor(0x3A63, 0x00);

    //Access LSC table //table1
    for(i=0; i<504; i++)
    {
        IMX258MIPI_write_cmos_sensor(0x4800+i, otp_ptr[i]);
#ifdef IMX258_LITEON_OTP_DEBUG
        LOG_INF("[mingji] otp_lsc_data[%d] == 0x%x\n",i,otp_ptr[i]);
#endif
    }
    //Turn on lsc
    IMX258MIPI_write_cmos_sensor(0x4500, 0x1f);
    IMX258MIPI_write_cmos_sensor(0x0700, 0x01);
    IMX258MIPI_write_cmos_sensor(0x3A63, 0x01);
