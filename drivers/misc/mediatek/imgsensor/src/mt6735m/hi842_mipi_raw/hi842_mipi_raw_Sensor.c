...
void write_HynixOtp_sensor_8(kal_uint32 addr, kal_uint32 para)
{
	char pu_send_cmd[4] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};
    iWriteRegI2C(pu_send_cmd, 3, imgsensor.i2c_write_id);
}

void write_HynixOtp_sensor_16(kal_uint32 addr, kal_uint32 para)
{
	char pu_send_cmd[4] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para >> 8),(char)(para & 0xFF)};
	iWriteRegI2C(pu_send_cmd, 4, imgsensor.i2c_write_id);
}
...
static void sensor_wb_gain_setting(void)
{
	...
	R_gain = 0x200 * RG_ratio_golden / RG_ratio_unit; // HJDDbg, JiangdePending 
	B_gain = 0x200 * BG_ratio_golden / BG_ratio_unit; // HJDDbg, JiangdePending 
...
// 20151202 
    ...
    printk("HJDDbgAWB use, RG_ratio_unit=0x%x(Gold=0x%x), BG_ratio_unit=0x%x(Gold=0x%x), R_gain=0x%x, G_gain=0x%x, B_gain=0x%x \n"
            , RG_ratio_unit, RG_ratio_golden, BG_ratio_unit, BG_ratio_golden, R_gain, G_gain, B_gain);
	
	write_cmos_sensor(0x0508, G_gain); // 20151202
	write_cmos_sensor(0x050a, G_gain); // 20151202
	write_cmos_sensor(0x050c, R_gain); // 20151202
	write_cmos_sensor(0x050e, B_gain); // 20151202
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    printk("HJDDbg, hi842-id, [get_imgsensor_id] ");
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
					printk("HJDDbg, hi842-id, i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
					read_Hi842_otp(); // HJDDbg, JiangdePending
					return ERROR_NONE;
				}	
				printk("HJDDbg, hi842-id, get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		printk("HJDDbg, hi842-open, SP \n");
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {				
				printk("HJDDbg, hi842-open, i2c write id: 0x%x, sensor id: 0x%x \n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
			printk("HJDDbg, hi842-open, open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x \n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
