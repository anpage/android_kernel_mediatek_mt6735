...
#include "ov5670mipi_cmk_Sensor.h"
...
extern signed char check_cmk_MID_ov5670(signed char index);
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 25,//unit , ns  //LINE<20160203>wangyanhui modify from 85
	...
	.margin = 16,           // Jiangde, 4-->16, Reddish when change from wide-angle to normal.  //sensor framelength & shutter margin  4 liunian
	.min_shutter = 16,		// Jiangde, 4-->16, Reddish when change from wide-angle to normal.  //min shutter
...
static void write_shutter(kal_uint16 shutter)
{
	...
	mdelay(2); // Jiangde, Reddish when change from wide-angle to normal.  //liunian  0ms
	// Update Shutter
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				{
          			*sensor_id = imgsensor_info.sensor_id+1;  //cmk camera return 0x5671
     			}
				return ERROR_NONE;
			}	
...
static kal_uint32 open(void)
{
	...
	sensor_init();
  otp_update_wb_cmk_ov5670(0x10D, 0x14F);//zyq add kaimujin typical for 5222 front camera
