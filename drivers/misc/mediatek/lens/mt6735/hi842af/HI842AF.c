...
#define AF_DRVNAME "HI842AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x19
#define PLATFORM_DRIVER_NAME "lens_actuator_hi842af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_hi842af"
...
inline static int getAFInfo(__user stHI842AF_MotorInfo * pstMotorInfo)
{
    stHI842AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stHI842AF_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stHI842AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stHI842AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stHI842AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case HI842AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stHI842AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case HI842AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stHI842AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case HI842AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case HI842AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case HI842AFIOC_T_SETMACROPOS :
...
*/
static void Hi842AF_CloseDelay(void)
{
    ...
    while (cur_position > last_position)
    {
		...
		LOG_INF("[Hi842AF] cur_position %6d \n", cur_position);
        msleep(15);
    }
    return;
}
...
static int __init HI842AF_i2C_init(void)
{
...
static void __exit HI842AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

/* ////////////////////////////////////////////////////////////// */
long HI842AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
/* 3.Only called once on last time. */
/* Q1 : Try release multiple times. */
int HI842AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
    ...
    if (*g_pAF_Opened == 2 && FACTORY_BOOT != get_boot_mode())
    {
        ...
        {
            moveAF(500);
            LOG_INF("HJDDbgAF, Wait--moveAF(%d) \n", g_u4CurrPosition);
            msleep(10);
        }
...
void HI842AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
...
// module_init(HI842AF_i2C_init);
// module_exit(HI842AF_i2C_exit);

// MODULE_DESCRIPTION("HI842AF lens module driver");
