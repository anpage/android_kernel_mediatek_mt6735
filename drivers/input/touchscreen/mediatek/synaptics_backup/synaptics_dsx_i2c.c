...
//@xuchunsheng changed start the keycode depend on the application in 26/08/2015
#define KEY_SY_SENSOR 250
//#define KEY_SY_SENSOR 251
//@xuchunsheng end in 26/08/2015
...
//        CTP_DBG("line = %d %s:g_bat_init_flag %d,rmi4_data->f01_ctrl_base_addr=%x\n",__LINE__,__func__,g_bat_init_flag,rmi4_data->f01_ctrl_base_addr);	
//        CTP_DBG("rmi4_data->f01_data_base_addr=%x\n",rmi4_data->f01_data_base_addr);	
        ...
        retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
	CTP_DBG("Failed to read device status, error = %d\n", retval);
	return retval;
	}
        chr_status = upmu_is_chr_det();
//        CTP_DBG("line = %d %s:chr_status %d,device_ctrl= %x\n",__LINE__,__func__,chr_status,device_ctrl);
        if(chr_status)
        {
...
//        CTP_DBG("line = %d %s:device_ctrl= %x\n",__LINE__,__func__,device_ctrl);        
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		CTP_DBG("Failed to issue reset command, error = %d\n",retval);
		return retval;
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	while (left_len > 0) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval < 0) {
				CTP_DBG(KERN_ERR"xxxx focal reads data error!! xxxx\n");
			}
...

{
	...
	while (left_len > 0) {	
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval < 0) {
				CTP_DBG(KERN_ERR"xxxxfocal write data error!! xxxx\n");
			}
			if (retval > 0) {
				break;
			} else {
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if(!gprDMABuf_va){
        ...
        if(!gprDMABuf_va){
			CTP_DBG("[Error] Allocate DMA I2C Buffer failed!\n");
	  }
        }
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		CTP_DBG("I2C read over retry limit\n");
		retval = -EIO;
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		CTP_DBG("I2C write over retry limit\n");
		retval = -EIO;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		//	rmi4_data->sensor_sleep = false;
           ...
           {
                 CTP_DBG("detected_gestures[0]=%d,gTGesture=%c\n",detected_gestures[0],gTGesture);
                 //@xuchunsheng changed start for touch gesture which is recognized by detected_gestures[3] in 25/08/2015
                 ...
                 //@xuchunsheng changed end in 25/08/2015
           }
...
if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
       ...
       else if(detected_gestures[0]==0x0B)
       {
		...
		{
                CTP_DBG("===synaptics_rmi4_f12_abs_report=====ic%d================\n",detected_gestures[2]);
                case 0x63:
                    gTGesture ='c';
                    input_report_key(SY_key_dev,KEY_SY_SENSOR, 1);
                    input_report_key(SY_key_dev,KEY_SY_SENSOR, 0);
	...
	if (extra_data->data15_size) {
		...
		CTP_DBG("Number of fingers to process = %d\n",fingers_to_process);
	}
		...
		if ((!Delicacy_status&&finger_status) | (Delicacy_status &&finger_Delicacy_status)) 
	{
		    ...
		    if(Delicacy_status &&(finger_Delicacy_status==6)&&(x>=10&&y>=110)&&(x<=700&&y<=525)) 
		      {
			CTP_DBG("==================tpd_down==========Delicacy_status====================finger=%d=x%d=y%d====\n",finger,x,y);
			...
			CTP_DBG("line = %d %s: Finger %d:status = 0x%02x x = %d y = %d wx = %d wy = %d\n",__LINE__,__func__, finger,finger_status,x, y, wx, wy);
			touch_count++;
		}
	}
////////////////////////////////
...
{           if(old_y >1100)
	...
	{
		CTP_DBG("==================tpd_up==========Delicacy_status=====finger==%d=======x%d=y%d=\n",finger,x,y);
		int ifnum =0;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		CTP_DBG("Failed to read button data registers\n");
		return;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	CTP_DBG("Function %02x reporting\n",fhandler->fn_number);
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		CTP_DBG("Failed to read interrupt status\n");
		return;
	}

	status.data[0] = data[0];
	if (status.unconfigured && !status.flash_prog) {
		CTP_DBG("spontaneous reset detected\n");
		retval = synaptics_rmi4_reinit_device(rmi4_data);
		if (retval < 0) {
			CTP_DBG("Failed to reinit device\n");
		}
		return;
	}
...
static irqreturn_t tpd_eint_handler(unsigned irq, struct irq_desc *desc)
{
	CTP_DBG("tpd_eint_handler:zx start\n");
...
static int touch_event_handler(void *data)
{
    ...
    CTP_DBG("touch_event_handler:zx start\n");
...
#endif
	CTP_DBG("Function %02x max x = %d max y = %d\n",
			fhandler->fn_number,
		...
		unsigned int intr_count)
{
	...
	//yaohua.li start	
		...
		CTP_DBG("[syna]:ctrl_base:%x, ctrl27_offset:%x\n", fhandler->full_addr.ctrl_base,extra_data->ctrl27_offset);
 		retval =  synaptics_rmi4_i2c_write(rmi4_data,
			fhandler->full_addr.ctrl_base+extra_data->ctrl27_offset,
			&gesture_enable,
			sizeof(gesture_enable));
		if (retval < 0) {
		CTP_DBG("Failed to enable single gesture\n");
		return;
		}		
	//yaohua.li end
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		CTP_DBG("Failed to alloc mem for function handle\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		CTP_DBG("Failed to read query registers\n");
		return retval;
	}
	...
	if (!f1a->button_control.txrx_map) {
		CTP_DBG("Failed to alloc mem for tx rx mapping\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_data_buffer) {
		CTP_DBG("Failed to alloc mem for data buffer\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		CTP_DBG("Failed to alloc mem for button map\n");
		return -ENOMEM;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (f1a->button_query.has_tx_rx_map) {
		...
		if (retval < 0) {
			CTP_DBG("Failed to read tx rx mapping\n");
			return retval;
		}
		...
		bool *was_in_bl_mode)
{
	...
	if (status.flash_prog == 1) {
		...
		CTP_DBG("In flash prog mode, status = 0x%02x\n",
				status.status_code);
	} else {
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		CTP_DBG("Failed to set configured\n");
		return;
	}
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (rmi_fd.fn_number == 0) {
				CTP_DBG("Reached end of PDT\n");
				break;
			}

			CTP_DBG("F%02x found (page %d)\n",
					rmi_fd.fn_number,
					page_number);
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					CTP_DBG("Failed to alloc for F%d\n",
							rmi_fd.fn_number);
					return retval;
				}
				...
				retval = synaptics_rmi4_check_status(rmi4_data,
						&was_in_bl_mode);
				if (retval < 0) {
					CTP_DBG("Failed to check status\n");
					return retval;
				}
...
flash_prog_mode:
	...
	CTP_DBG("Number of interrupt registers = %d\n",	rmi4_data->num_of_intr_regs);
	...
	if (rmi->manufacturer_id != 1) {
		CTP_DBG("Non-Synaptics device found, manufacturer ID = %d\n",rmi->manufacturer_id);
	}
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			CTP_DBG("Interrupt enable mask %d = 0x%02x\n",	ii, rmi4_data->intr_mask[ii]);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->input_dev == NULL) {
		CTP_DBG("Failed to allocate input device\n");
		retval = -ENOMEM;
	...
	if (retval) {
		CTP_DBG("Failed to register input device\n");
		...
		bool insert)
{
	...
	if (insert) {
		...
		if (!exp_fhandler) {
			CTP_DBG("Failed to alloc mem for expansion function\n");
			goto exit;
		}
...
int sy_open(struct inode *inode, struct file *filp){ 
      ...
      CTP_DBG("[SY]into sy_open\n");
      if (g_pts == NULL)  
	  		CTP_DBG("private_ts is NULL~~~");
...
static ssize_t sy_write(struct file *filp, const char *buff, size_t count, loff_t *offp){  
    int ret;
    char *tmp;
    CTP_DBG("[SY]into elan_iap_write\n");
    if (count > 8192)
        count = 8192;
...
#endif
{
	...
	if (!rmi4_data) {
		CTP_DBG("Failed to alloc mem for rmi4_data\n");
		return -ENOMEM;
	}
	...
	//yaohua.li
     ...
     if (misc_register(&rmi4_data->firmware) < 0)
               CTP_DBG("mtk-tpd:[SY] misc_register failed!!\n");
     else
               CTP_DBG("[SY] misc_register finished!!\n"); 
	...
	if (retval < 0) {
		CTP_DBG("Failed to set up input device\n");
...
//	  strcpy(Tg_buf,"mcs");
	...
	{
		CTP_DBG("[STP]SY_key_dev register : fail!\n");
	}else
	{
		CTP_DBG("[STP]SY_key_dev register : success!!\n");
	} 
//end by yaohua.li
	...
	if ( IS_ERR(thread) ) {
		...
		CTP_DBG(" %s: failed to create kernel thread: %d\n",__func__, retval);
	}

	tpd_irq_registration();
	retval = synaptics_rmi4_irq_enable(rmi4_data, true);
	if (retval < 0) {
		CTP_DBG("Failed to enable attention interrupt\n");
		goto err_enable_irq;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			CTP_DBG("Failed to create sysfs attributes\n");
			goto err_sysfs;
		}
	}
		...
		bool enable)
{
	...
	CTP_DBG("=====synaptics_rmi4_f12_wg===start===\n");
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
	     if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
	     {
		CTP_DBG("=======synaptics_rmi4_f12_wg======find====\n");
			break;
             }
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(reporting_control));
	if (retval < 0) {
		CTP_DBG("Failed to change reporting mode\n");
		return;
	}
	CTP_DBG("======enables==%d===========\n",enable);
	if (enable)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
	retval = synaptics_rmi4_i2c_write(rmi4_data,fhandler->full_addr.ctrl_base + offset,reporting_control,
			sizeof(reporting_control));
	if (retval < 0) {
		CTP_DBG("========%s: Failed to change reporting mode\n",__func__);
		return;
	}
//yaohua.li
CTP_DBG("=====synaptics_rmi4_f12_wg===end===\n");
	return;
}
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		CTP_DBG("Failed to enter sleep mode\n");
		rmi4_data->sensor_sleep = false;
		return;
	}
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		CTP_DBG("Failed to wake from sleep mode\n");
		rmi4_data->sensor_sleep = true;
		return;
	}
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	CTP_DBG("synaptics_rmi4_early_suspend");
...
exit:
        //@xuchunsheng changed start for leakage current when tp enter in sleep in 08/29/2015
        //rmi4_data->sensor_sleep = true;
	rmi4_data->sensor_sleep = false;
        //@xuchunsheng changed end in 08/29/2015
	return;
}
...
static void synaptics_rmi4_late_resume(struct early_suspend *h)
{
	...
	if (rmi4_data->staying_awake)
		return;
             CTP_DBG("synaptics_rmi4_late_resume");
	if (rmi4_data->enable_wakeup_gesture) {
...
{    
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
	     if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
	     {
		CTP_DBG("=======set_tgesture_enable======find====\n");
			break;
             }
 	}
	       extra_data = (struct synaptics_rmi4_f11_extra_data *)fhandler->extra;
...
CTP_DBG("[syna]:ctrl_base:%x\n", fhandler->full_addr.ctrl_base);
		...
		if (retval < 0) 
               {
		   CTP_DBG("%s: Failed to enable single gesture\n",__func__);
		   return;
	   }	
#endif
	//yaohua.li end
 /*     CTP_DBG("==========set_tgesture_enable==============\n");
 	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
	synaptics_rmi4_wakeup_gesture(rmi4_data, true);*/
...
{    
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
	     if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
	     {
		CTP_DBG("=======et_tgesture_unenable======find====\n");
			break;
             }
 	}
	       extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
...
{    
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
		...
		CTP_DBG("===========synaptics_rmi4_f12_abs_report==TP_Delicacy===================true start====\n");
		CTP_DBG("[syna]:ctrl_base:%x, ctrl23_offset:%x\n", fhandler->full_addr.ctrl_base,extra_data->ctrl23_offset);
 		retval =  synaptics_rmi4_i2c_write(rmi4_data,fhandler->full_addr.ctrl_base+extra_data->ctrl23_offset,&Delicacy,sizeof(Delicacy));
		if (retval < 0) 
		{
			//CTP_DBG(&(rmi4_data->input_dev->dev),"%s: Failed to enable single gesture\n",__func__);
	                CTP_DBG("===========synaptics_rmi4_f12_abs_report==TP_Delicacy===================true end====\n");
			return 0;
		}
      ...
      CTP_DBG("==========set__Delicacy_enable==============\n");
	return;
}
void set_Delicacy_unenable()
{
       CTP_DBG("==========set__Delicacy_unenable==============\n");
	int retval = 0;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
        struct synaptics_rmi4_f12_extra_data *extra_data; 	
...
void set_sleep_unenable()
{
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
		...
		CTP_DBG("===========synaptics_rmi4_f12_abs_report==set_sleep===================true start====\n");
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (rmi4_data->staying_awake)
		return 0;
	CTP_DBG("============synaptics_rmi4_suspend===%d====\n",rmi4_data->enable_wakeup_gesture);
	bTPGesturetemp =bEnTGesture;
             CTP_DBG("============synaptics_rmi4_suspend==bEnTGesture=%d====\n",bEnTGesture);
	if (bTPGesturetemp) {	
	synaptics_chip_reset();
	...
	else
	{
	    if (rmi4_data->enable_wakeup_gesture) 
        {		
	        CTP_DBG("====rmi4_data->enable_wakeup_gesture===%d==\n",rmi4_data->enable_wakeup_gesture);
	        synaptics_rmi4_wakeup_gesture(rmi4_data, false);
        }
        ...
        msleep(200);
        CTP_DBG("============synaptics_rmi4_resume===set_tgesture_unenable==\n");	
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(g_dev);
         CTP_DBG("============synaptics_rmi4_resume========\n");
	if (rmi4_data->staying_awake)
		return 0;
        CTP_DBG("============synaptics_rmi4_resume===%d====\n",rmi4_data->enable_wakeup_gesture);
	...
	else
	{
		...
		msleep(50);
		enable_irq(touch_irq);
		CTP_DBG("============synaptics_rmi4_resume====set_tgesture_enable==\n");
	}
	synaptics_rmi4_sensor_wake(rmi4_data);
	synaptics_rmi4_irq_enable(rmi4_data, true);
	retval = synaptics_rmi4_reinit_device(rmi4_data);
...
///////////////////////////////////////////////////////////////////////////////
		if (1==Delicacy_status)
		{
			...
			CTP_DBG("============set_tgesture_unenable=====================\n");
			set_Delicacy_enable();			
			CTP_DBG("============set_Delicacy_enable=====================\n");
...
static int tpd_local_init(void)
{
	...
	{
		CTP_DBG("tangjie Error unable to add i2c driver.\n");
		return -1;
	}
#ifdef FTM_BUTTON  //TPD_HAVE_BUTTON     
...
char* get_synaptics_TpName( void )
{
   ...
   CTP_DBG("=======tpd_desc_info=%s===",tpd_desc_info);
...
void synaptics_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    ...
    unsigned char  name[7]={"iTOUCH"},index=0;
     CTP_DBG("=======synaptics_tpd_get_fw_vendor_name=%s===",product_id);
    if(product_id)
    {
...
{
    CTP_DBG("synaptics_rmi4_init i2c=%d",TPD_I2C_BUS);
	if(tpd_driver_add(&synaptics_rmi4_driver) < 0){
		CTP_DBG("Fail to add tpd driver\n");
		return -1;
	}
