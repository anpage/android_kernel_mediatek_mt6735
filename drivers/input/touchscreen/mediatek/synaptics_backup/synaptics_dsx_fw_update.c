...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	CTP_DBG("device_fw_id=%d",device_fw_id);
	...
	CTP_DBG("image_fw_id=%ld",image_fw_id);
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			...
			config_id[2],
			config_id[3]);
	CTP_DBG("Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x",config_id[0],config_id[1],config_id[2],config_id[3]);
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev,
			...
			fwu->config_data[2],
			fwu->config_data[3]);
	CTP_DBG("Image config ID = 0x%02x 0x%02x 0x%02x 0x%02x",fwu->config_data[0],fwu->config_data[1],fwu->config_data[2],fwu->config_data[3]);
...
int update_FW(void)
{
 ...
 printk("geroge synaptics  ftm_force_update  = %d\n",ftm_force_update);
	if(ftm_force_update){ 
...
static int fwu_start_reflash(void)
{
	...
	CTP_DBG("Start of reflash processxxxx1\n");

	if (fwu->ext_data_source) {
		fw_image = fwu->ext_data_source;
	} else {
		strncpy(fwu->image_name, FW_IMAGE_NAME, MAX_IMAGE_NAME_LEN);
		CTP_DBG("Requesting firmware image %s\n",fwu->image_name);

		retval = request_firmware(&fw_entry, fwu->image_name,
				&fwu->rmi4_data->i2c_client->dev);
		if (retval != 0) {
			CTP_DBG("Firmware image %s not available\n",fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

		CTP_DBG("Firmware image size = %ld\n",fw_entry->size);
	...
	if (fwu->bl_version != header.bootloader_version) {
		CTP_DBG("Bootloader version mismatch\n");
		retval = -EINVAL;
		goto exit;
	}
...
exit:
	...
	CTP_DBG("End of reflash process\n");
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	fw_data = FW_IMG_DEFAULT;
	synaptics_fw_updater(fw_data);
        //@xuchunsheng added started for vertion number can not update first time when updating the firmware with dijing tp in 09/29/2015
        synaptics_read_fw_version();
        synaptics_get_fw_version();
        //@xuchunsheng added end in 09/29/2015
	return;
}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	synaptics_read_fw_version();
        //@xuchunsheng added started for vertion number can not update first time when updating the firmware with dijing tp in 09/29/2015
        synaptics_get_fw_version();
        //@xuchunsheng ended in 09/29/2015
	fwu->fwu_workqueue = create_singlethread_workqueue("fwu_workqueue");
	INIT_DELAYED_WORK(&fwu->fwu_work, fwu_startup_fw_update_work);
	queue_delayed_work(fwu->fwu_workqueue,
			&fwu->fwu_work,
