...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	else if((writelen > 8)&&(NULL != g_dma_buff_pa))
	{
		for (i = 0; i < writelen; i++)
			((char*)g_dma_buff_pa)[i] = writebuf[i];
...
static char *tpd_get_vendor_name(int vendor_id)
{
	switch(vendor_id){
...
*/
	...
	retval = fts_read_reg(i2c_client, 0xA8, &panel_vendor);
	if (retval < 0) {
		TPD_DEBUG("i2c error, ret=%d\n", ret);
		return -1;
	}
	sprintf(tpd_desc, "%s", tpd_get_vendor_name(panel_vendor));
...
static void tpd_get_fw_version(char * fw_vendor_numb)
{
	int ret;
	u16 fw_version = 0;
	ret = fts_read_reg(i2c_client, 0xA6, &fw_version);
	if (ret < 0){
		TPD_DEBUG("i2c error, ret=%d\n", ret);
		return -1;
	}
...
static void tpd_get_fw_vendor_name(char * fw_vendor_name)
{
...
static struct tpd_driver_t tpd_device_driver = {
	...
	.tpd_get_fw_version = tpd_get_fw_version,
	.tpd_get_fw_vendor_name = tpd_get_fw_vendor_name,
};
